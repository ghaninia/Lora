                ...
                <div class="row">
                    ...
                    <div class="col-lg-3">
                        <div class="submit-field">
                            <h5>{{ trans('dash.table.username') }}</h5>
                            <input class="{{ $errors->has('username') ? 'border-danger' : null }}" autocomplete="off" name="username" value="{{ request('username') }}">
                        </div>
                        <div class="submit-field">
                            <h5>{{ trans('dash.table.status') }}</h5>
                            <select class="selectpicker margin-top-0" name="status">
                                <option value="">{{ trans('dash.items.select_one_type') }}</option>
                                @foreach($statusPayment as $status)
                                    <option @if( request('status') == $status) selected @endif value="{{ $status }}">{{ trans("dash.status.payment.{$status}") }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-3">
                        <div class="submit-field">
                            <h5>{{ trans('dash.table.tracking_code') }}</h5>
                    ...
                    <div class="col-lg-3">
                        <div class="submit-field">
                            <h5>{{ trans('dash.table.discount_code') }}</h5>
                        ...
                        <button class="button ripple-effect move-on-hover full-width margin-top-55">
                            <span>{{ trans('dash.table.filter_payments') }}</span>
                        </button>
                    </div>
            ...
            <table class="table table-padded">
                <thead>
                    <tr>
                        ...
                        <th>{{ trans('dash.table.discount_code') }}</th>
                        <th class=" text-center">{{ trans('dash.table.transaction_id') }}</th>
                        <th class="text-center">{{ trans('dash.table.tracking_code') }}</th>
                        <th>{{ trans('dash.table.created_at') }} {{ SortBy('created_at') }}</th>
                    ...
                    @foreach($payments as $payment)
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="smaller lighter">{{ $payment->created_at->format( config('dash.format_date') ) }}</span>
                            </td>
