...
@section('main')
        ...
        <form action="{{ route('dashboard.role.index') }}">
            <div class="switch-container">
                <label class="switch">
                    ...
                    <span class="switch-text">{{ trans('dash.roles.just_default') }}</span>
                </label>
            </div>
                    ...
                    <div class="job-listing-details">
                        ...
                        <div class="job-listing-description">
                            <h4 class="job-listing-company">
                                {{ $role->name }}
                                @if($role->default)
                                    <span class="verified-badge" title="{{ trans('dash.roles.default.text') }}" data-tippy-placement="top"></span>
                                @endif
                            </h4>
                    ...
                    <div class="job-listing-footer">
                        <span class="delete-icon {{ $role->default ? '' :  'deleted' }}"
                            @if(!$role->default)
                                role="swal"
                                href="#"
                                data-message="{{ trans('dash.questions.role_delete') }}"
                                data-url="{{ route('dashboard.role.destroy' , $role->id ) }}"
                                data-action="delete"
                            @endif
                          title="{{ trans('dash.roles.delete.text') }}"
                          data-tippy-placement="top"
                        >
                        </span>
                        <a title="{{ trans('dash.roles.edit.text') }}"
                           data-tippy-placement="top"
    ...
    <a class="btn--fixed btn--action" href="{{ route('dashboard.role.create') }}" title="{{ trans('dash.items.create_new') }}" data-tippy-placement="right">
