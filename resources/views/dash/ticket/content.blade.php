                    ...
                    <div class="message-bubble-inner">
                        ...
                        <div class="message-text">
                            ...
                            @if(!! $message->subject && !! $message->priority )
                                <ul class="subjects">
                                    <li><strong>{{ trans("dash.tickets.priority.text") }} :</strong>{{ trans("dash.tickets.priority.{$message->priority}") }}</li>
                                    <li><strong>{{ trans("dash.items.status") }} :</strong>
                                        @if($message->status == 'enable')
                                            {{  trans("dash.status.true") }}
                                        @else
                                            {{  trans("dash.status.false") }}
                                        @endif
                                    </li>
                                    <li><strong>{{ trans('dash.tickets.subject') }} :</strong>{{ $message->subject  }}</li>
                                    <li><strong>{{ trans('dash.tickets.traking_code') }} :</strong>{{ $trakingCode }}</li>
                                </ul>
                            @endif
                            <p>
                                {{ $message->message }}
...
@if( $information->status == 'enable' )
    <form method="POST" class="message-reply replayMessage" action="{{ route('dashboard.ticket.replay') }}">
        ...
        <textarea required cols="1" id="message" name="message" rows="1" placeholder="{{ trans('dash.tickets.replay') }}" data-autoresize></textarea>
        <button class="button ripple-effect">{{ trans('dash.tickets.send') }}</button>
    </form>
@endif